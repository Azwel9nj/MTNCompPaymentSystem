...
class AgentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Toastr::success(\App\CentralLogics\translate('Agent Added Successfully!'));
        return back();
    }
...
//            'password' => 'required|min:4|max:4',
        ...
        if ($request->has('image')) {
            $image_name = Helpers::update('agent/', $agent->image ?? '', 'png', $request->file('logo'));
        } else {
...
//        $agent->phone = $agent->phone;
        ...
        $agent->type = 1;    //['Admin'=>0, 'Agent'=>1, 'Customer'=>2]
